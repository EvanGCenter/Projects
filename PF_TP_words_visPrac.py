...
def PF_TP_visual_prac():
    
    # Create a unique filename for the experiment data
    if not os.path.isdir(datapath):
        os.makedirs(datapath)
    data_fname = 'PF_TP_words_visual_prac' + exp_info['participant'] + '_' + exp_info['date']
    ...
    durations = [int(d*.06) for d in durations]
    comp_frames = list(np.repeat(durations,5))
    rnd.shuffle(comp_frames)
    stan_frames = list(np.repeat(int(standard*.06),10))
