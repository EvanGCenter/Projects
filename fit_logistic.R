...
for (i in 1:n_files) {
  ...
  objSi$acc <- ifelse(objSi$time1 == median(objSi$time1), NaN, objSi$acc)
  
  fail <- objSi$cjl == "None"
  objSi$cjl[fail] <- NaN
  objSi$cjl <- as.integer(objSi$cjl)
  
  yg <- objSi$cjl[objSi$order<3]
  xg <- objSi$time1[objSi$order<3]

  yb <- objSi$cjl[objSi$order>2]
  xb <- objSi$time1[objSi$order>2]
