...
deets.L <- read.csv('C:/Users/Evan/Documents/legitwords.csv')
LMNF.L <- as.vector(deets.L$Log.Max.Neighbor.Frequency)
words.L <- as.vector(deets.L$Item)
deets.P <- read.csv('C:/Users/Evan/Documents/pseudowords.csv')
LMNF.P <- as.vector(deets.P$Log.Max.Neighbor.Frequency)
words.P <- as.vector(deets.P$Item)

neighbor.vector <- c(deets.L$Log.Max.Neighbor.Frequency,deets.P$Log.Max.Neighbor.Frequency)
split <- median(na.omit(neighbor.vector))
...
for (i in 1:n_files) {
  ...
  objSi$wordStem <- gsub("_.*$","",objSi$comp)
  L.key <- match(objSi$wordStem,words.L)
  P.key <- match(objSi$wordStem,words.P)
  full.key <- pmin(L.key,P.key,na.rm = TRUE)
  objSi$orthoL <- LMNF.L[L.key]
  objSi$orthoP <- LMNF.P[P.key]
  objSi$ortho <- pmin(objSi$orthoL,objSi$orthoP,na.rm = TRUE)
  ...
  yg <- objSi$cjl[objSi$ortho>split]
  xg <- objSi$comp_frames[objSi$ortho>split]

  yb <- objSi$cjl[objSi$ortho<split]
  xb <- objSi$comp_frames[objSi$ortho<split]
