...
def PF_TP_audio():
    ...
    data_fname = 'PF_TP_words_audio' + exp_info['participant'] + '_' + exp_info['date']
...
#    win.flip()
    ...
    comp_sounds = [pygame.mixer.Sound(os.path.join(legit_path,f)) if f in all_words else pygame.mixer.Sound(os.path.join(pseudo_path,f)) for f in comp_list]
    stan_sounds = [pygame.mixer.Sound(os.path.join(pseudo_path,f)) if f in all_pseudo else pygame.mixer.Sound(os.path.join(legit_path,f)) for f in stan_list]
...
#    n_trials = 10    
    ...
    stim_order = []
    for trial_num, stan, comp, stan_id, comp_id, stan_frames, comp_frames, t_order in zip(range(len(stan_list)), stan_list, comp_list, stan_id, comp_id, stan_frames, comp_frames, t_order):
        stim_order.append({'trial_num': trial_num, 'stan': stan, 'comp': comp, 'stan_id': stan_id, 'comp_id': comp_id, 'stan_frames': stan_frames, 'comp_frames': comp_frames, 't_order': t_order})
...
#    keys = event.waitKeys(keyList=['space', 'escape'])
        ...
        while len(keys) == 0 and rt_clock.getTime() < timelimit:
            if trial['t_order'] == 0: #comp -> stan
                for frame in range(trial['comp_frames']):
                    if frame == 0: pygame.mixer.Sound.play(comp_sounds[p]) #play comparison sound
                    fix1.draw()
                    fix2.draw()
                    win.flip()
    ...
    #            core.wait(int(trial['comp_id'][-7:-4])/1000.0)
                ...
                for frame in range(trial['stan_frames']):
                    if frame == 0: pygame.mixer.Sound.play(stan_sounds[p]) #play standard sound
                    fix1.draw()
                    fix2.draw()
                    win.flip()
